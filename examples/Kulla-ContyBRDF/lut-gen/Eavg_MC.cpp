...
int main() {
	...
	else 
    {
        ...
        auto data = std::make_unique<uint8_t[]>(resolution * resolution * 3);
		...
		for (int i = 0; i < resolution; i++) 
        {
			...
			for (int j = 0; j < resolution; j++) 
            {
                ...
                setRGB(i, j, 0.0, data.get());
			}

            for(int k = 0; k < resolution; k++)
            {
                setRGB(i, k, Eavg, data.get());
            }
		...
		stbi_write_png("GGX_Eavg_LUT.png", resolution, resolution, channel, data.get(), 0);
	}
