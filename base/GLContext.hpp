...
struct EventListenerTraits{
  ...
  static KeyboardEventCallback KeyboardEvent;
  static FileDropEventCallback FileDropEvent;
  static FramebufferResizeEventCallback FramebufferResizeEvent;
};

inline MouseEventCallback EventListenerTraits::MouseEvent = MouseEventCallback();
inline ScrollEventCallback EventListenerTraits::ScrollEvent=ScrollEventCallback();
inline KeyboardEventCallback EventListenerTraits::KeyboardEvent = KeyboardEventCallback();
inline FileDropEventCallback EventListenerTraits::FileDropEvent = FileDropEventCallback();
inline FramebufferResizeEventCallback EventListenerTraits::FramebufferResizeEvent = FramebufferResizeEventCallback();
...
inline std::string GetGLErrorStr(GLenum gl_error){
  ...
  case GL_INVALID_INDEX: error = "GL_INVALID_INDEX"; break;
...
#define GL_EXPR(expr)                                                          \
  do {                                                                         \
    ...
    while ((gl_error = glGetError()) != GL_NO_ERROR) {                         \
      std::cout << "GL error " << GetGLErrorStr(gl_error) << " before call "   \
                << #expr << " at line " << __LINE__ << " in file " << __FILE__ \
                << std::endl;                                                  \
      count++;                                                                 \
      if (count > 10)                                                          \
        break;                                                                 \
    ...
    count = 0;                                                                 \
    while ((gl_error = glGetError()) != GL_NO_ERROR) {                         \
      std::cout << "Calling " << #expr << " caused GL error "                  \
                << GetGLErrorStr(gl_error) << " at line " << __LINE__          \
...
#define GL_CHECK                                                               \
  do {                                                                         \
    ...
    int count = 0;                                                             \
    while ((gl_error = glGetError()) != GL_NO_ERROR) {                         \
      std::cout << "GL error " << GetGLErrorStr(gl_error) << " before line "   \
...
template<typename GLContextImpl,typename GLAPILoaderImpl,GLObjectType type>
class GLObject final{
  ...
  GLObject(std::shared_ptr<Context> ctx,uint32_t object)
      :gl_context(std::move(ctx)),m_object(object){
...
class GLContext final
    :public std::enable_shared_from_this<GLContext<GLContextImpl,GLAPILoaderImpl>>,
     public GLContextImpl,public GLAPILoaderImpl
{
...
public:
  ...
  using GLObject_T = GLObject<GLContextImpl,GLAPILoaderImpl,type>;
  ...
  auto CreateRenderbuffer() {
    uint32_t handle = 0;
    GL_EXPR(glGenRenderbuffers(1,&handle));
    GL_EXPR(glBindRenderbuffer(GL_RENDERBUFFER,handle));
